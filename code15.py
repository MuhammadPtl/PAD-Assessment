...
# Date - 12/06/24
# Purpose - To Create a code to create a GUI for a party hire page allowing the user to input their details and return using their receipt number
# ----------------------
...
logo_image = tkinter.PhotoImage(file="image1.png")

# Resize the logo to better fit the page.
resized_image = logo_image.subsample(2, 2)
img_label = tkinter.Label(window, image=resized_image)
img_label.image = resized_image  
img_label.place(x=120, y=1, width=220, height=100)
...
def hire_a():
    ...
    item_combobox = ttk.Combobox(window_hire, values=["Balloons", "Plates", "Spoons", "Forks"], state='readonly')
    ...
    # Button to append details (trigger input validation and receipt).
...
# Function to close the GUI program.
def quit_a():
...
hire_button.place(x=175, y=105, width=100)
...
return_button.place(x=175, y=145, width=100)
...
viewreceipt_button.place(x=175, y=185, width=100)
...
quit_button.place(x=175, y=225, width=100)
